resource "aws_cloudfront_distribution" "forpractice" {
  enabled = true
  ...
  records = [
    {
      ...
      type = "A"
      alias = {
        name    = aws_cloudfront_distribution.forpractice.domain_name
        zone_id = aws_cloudfront_distribution.forpractice.hosted_zone_id
      }
    }
  ]
}